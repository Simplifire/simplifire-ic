                ...
                <div class="mt-4 card card-body">
                    <div class="row">
                        ...
                        <div class="col-lg-6 d-flex align-items-center justify-content-end">
                                ...
                                <label>Author: <span class="badge rounded-pill bg-dark">{{ author?.email }}</span></label>
                                
                                <button v-if="!sharedWith"
                                    type="button"
                                    name="button"
                                    class="m-2 btn bg-gradient-primary ms-2"
                                    data-bs-toggle="modal"
                                    data-bs-target="#exampleModal"
                                    @click="shareDocument">Share</button>
                                
                                <label v-else>Shared with: <span class="badge rounded-pill bg-dark">{{ sharedWith?.email }}</span></label>
        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-body">
                <div class="row row-cols-4 row-cols-md-1 g-3">
                    <div class="row">
                        <user-card v-for="user in users.filter(u => u.id !== this.$store.state.user_id && u.id !== this.author?.id)"
                            :key="user.id"
                            :user="user"
                            @share="shareDocument"></user-card>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async shareDocument(userId) {   
            await rust_simplifire.add_user_document(parseInt(this.$route.params.id), userId, "counter_party");
